...
class Ui_Elgamal_demo(object):
    ...
    def retranslateUi(self, Elgamal_demo):
        ...
        self.btnSend.setText(_translate("Elgamal_demo", "Send Ciphenrtext"))
    ...
    def encrypt(self):
        ...
        # E_Kpub(x,k)=(y1,y2): y1 = Alpha^k mod p ; y2= x*Beta^k mod p
    ...
    def decrypt(self):
        ...
        #D_Kpir(y1,y2) = y2*((y1^a)^-1) mod p  = ( (y2 mod p)* ((y1^a mod p)^-1 mod p) ) mod p
        res = self.powNegativeOne(self.powMod(self.y1,self.a,self.p),self.p)   # (y1^a mod p)^-1 mod p)
